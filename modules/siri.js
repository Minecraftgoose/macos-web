...
(function(){
    ...
    // API 配置已集中到 modules/config.js（window.SIRI_API_CONFIG）
    const SIRI_CFG = window.SIRI_API_CONFIG || {};
    const POLLINATIONS_API_KEY = SIRI_CFG.pollinations?.apiKey || '';
    const POLLINATIONS_ENDPOINT = SIRI_CFG.pollinations?.endpoint || 'https://gen.pollinations.ai/v1/chat/completions';
    
    const ZHIPU_API_KEY = SIRI_CFG.zhipu?.apiKey || '';
    const ZHIPU_ENDPOINT = SIRI_CFG.zhipu?.endpoint || 'https://open.bigmodel.cn/api/paas/v4/chat/completions';
    const AGENT_MODEL = SIRI_CFG.zhipu?.model || 'glm-4-flash';
...
**可用函数**：
openApp, closeWindow, minimizeWindow, restoreWindow, setDarkMode, setBrightness, lockScreen, sleep, showNotification, showProgress, updateProgress, showStatus, hideIsland, searchWeb
        ...
        try {
            switch (funcName) {
                case 'openApp': {
                    ...
                    if (window.openApp) window.openApp(appName, { skipLastApps: args.skipLastApps === true });
                    return `打开了${appName}`;
                }
                case 'closeWindow': {
                    if (args.appName) {
    ...
    async function handleLocalBatchCommand(text) {
        ...
        if (/(打开|启动)\s*(所有|全部|每一个)\s*(应用|程序|软件)/.test(lowerText) ||
            ...
            for (const app of apps) {
                // 批量打开不记录到"上次打开"，避免污染启动恢复列表
                const result = await executeFunctionCall('openApp', { appName: app, skipLastApps: true });
                results.push(result);
