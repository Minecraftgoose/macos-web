...
const AnimationManager = {
    ...
    setupGlobalListeners() {
        // 减弱动画偏好支持（默认关闭，见 config.js 的 window.MACOS_CONFIG.reducedMotion）
        const cfg = window.MACOS_CONFIG || {};
        if (cfg.reducedMotion !== true) {
            // 忽略系统 prefers-reduced-motion，避免过渡/动画被压成 0.01ms 导致面板开关异常
            return;
        }
